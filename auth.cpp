...
void KeyAuth::api::init()
{
    ...
    if (!constantTimeStringCompare(ss_result.str().c_str(), signature.c_str(), sizeof(signature).c_str())) { // check response authenticity, if not authentic program crashes
        error(XorStr("Signature checksum failed. Request was tampered with or session ended most likely. & echo: & echo Message: ").c_str() + message);
    }
...
static size_t header_callback(char* buffer, size_t size, size_t nitems, void* userdata)
{
    ...
    if (temp.substr(0, 9) == XorStr("signature").c_str()) {
...
void KeyAuth::api::web_login()
{
    ...
    if (result == ERROR_INVALID_PARAMETER) {
        MessageBoxA(NULL, XorStr("The Flags parameter contains an unsupported value.").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }
    if (result != NO_ERROR) {
        MessageBoxA(NULL, XorStr("System error for Initialize").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }
    ...
    if (result == ERROR_REVISION_MISMATCH) {
        MessageBoxA(NULL, XorStr("Version for session invalid").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_INVALID_PARAMETER) {
        MessageBoxA(NULL, XorStr("pServerSessionId parameter is null").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result != NO_ERROR) {
        MessageBoxA(NULL, XorStr("System error for HttpCreateServerSession").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }
    ...
    if (result == ERROR_INVALID_PARAMETER) {
        MessageBoxA(NULL, XorStr("Url group create parameter error").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result != NO_ERROR) {
        MessageBoxA(NULL, XorStr("System error for HttpCreateUrlGroup").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }
    ...
    if (result == ERROR_REVISION_MISMATCH) {
        MessageBoxA(NULL, XorStr("Wrong version").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_INVALID_PARAMETER) {
        MessageBoxA(NULL, XorStr("Byte length exceeded").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_ALREADY_EXISTS) {
        MessageBoxA(NULL, XorStr("pName already used").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_ACCESS_DENIED) {
        MessageBoxA(NULL, XorStr("queue access denied").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_DLL_INIT_FAILED) {
        MessageBoxA(NULL, XorStr("Initialize not called").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result != NO_ERROR) {
        MessageBoxA(NULL, XorStr("System error for HttpCreateRequestQueue").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }
    ...
    if (result == ERROR_INVALID_PARAMETER) {
        MessageBoxA(NULL, XorStr("Invalid parameter").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result != NO_ERROR) {
        MessageBoxA(NULL, XorStr("System error for HttpSetUrlGroupProperty").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    // Add URLs to URL group.
    PCWSTR url = XorStr(L"http://localhost:1337/handshake").c_str();
    result = HttpAddUrlToUrlGroup(groupId, url, 0, 0);

    if (result == ERROR_ACCESS_DENIED) {
        MessageBoxA(NULL, XorStr("No permissions to run web server").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_ALREADY_EXISTS) {
        MessageBoxA(NULL, XorStr("You are running this program already").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_INVALID_PARAMETER) {
        MessageBoxA(NULL, XorStr("ERROR_INVALID_PARAMETER for HttpAddUrlToUrlGroup").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result == ERROR_SHARING_VIOLATION) {
        MessageBoxA(NULL, XorStr("Another program is using the webserver. Close Razer Chroma mouse software if you use that. Try to restart computer.").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }

    if (result != NO_ERROR) {
        MessageBoxA(NULL, XorStr("System error for HttpAddUrlToUrlGroup").c_str(), XorStr("Error").c_str(), MB_ICONEXCLAMATION);
        __fastfail(0);
    }
    ...
    while (going == true)
    {
        ...
        std::string user = get_str_between_two_str(myVarS, XorStr("?user="), XorStr("&"));
        std::string token = get_str_between_two_str(myVarS, XorStr("&token="), XorStr(""));
...
void KeyAuth::api::button(std::string button)
{
    ...
    output = std::wstring(XorStr(L"http://localhost:1337/").c_str()) + output;
...
void error(std::string message) {
    std::string command = XorStr("start cmd /C color b && title Error && echo ").c_str() + message + XorStr(" && timeout /t 5").c_str();
...
std::string checksum()
{
    auto exec = [&](const char* cmd) -> std::string
        {
            ...
            std::string result;
            std::unique_ptr<FILE, decltype(&_pclose)> pipe(_popen(cmd, XorStr("r").c_str()), _pclose);
            if (!pipe) {
    ...
    return exec((XorStr("certutil -hashfile \"").c_str() + std::string(rawPathName) + XorStr("\" MD5 | find /i /v \"md5\" | find /i /v \"certutil\"")).c_str());
}
...
void checkInit() {
    if (!initalized) {
        error(XorStr("You need to run the KeyAuthApp.init(); function before any other KeyAuth functions").c_str());
    }
}
...
void modify()
{
    ...
    while (true)
    {
        ...
        {
            error(XorStr("check_section_integrity() failed, don't tamper with the program.").c_str());
        }
        ...
        if (!LockMemAccess())
        {
            error(XorStr("LockMemAccess() failed, don't tamper with the program.").c_str());
        }
        // code submitted in pull request from https://github.com/BINM7MD
        ...
        if ((DWORD64)Instruction == 0xE9) {
            error(XorStr("Pattern checksum failed, don't tamper with the program.").c_str());
        }
        Sleep(50);
    }
}